...
public class Author extends JFrame {
	...
	public Author() {
		...
		table.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
                ...
                if (selectedRow != -1) {
                	...
                	nationf.setText(table.getValueAt(selectedRow, 3).toString());
                	countf.setText(table.getValueAt(selectedRow, 4).toString());
		...
		table.setModel(new DefaultTableModel(
			...
			new String[] {
				"author id", "first name", "last name", "nationality", "publish count", "date of birth"
			}
		));
			...
			public void actionPerformed(ActionEvent e) {
				try (Connection con = DriverManager.getConnection(url, myname, mypassword)) {
				    ...
				    if (!searchf.getText().isEmpty()) {
				    	...
				    	query = "SELECT * FROM author WHERE LOWER(first_name) LIKE LOWER(?) OR LOWER(last_name) LIKE LOWER(?)";;
				        ...
				        try (PreparedStatement pstmt = con.prepareStatement(query)) {
				            ...
				            try (ResultSet rst = pstmt.executeQuery()) {
				                ...
				                while (rst.next()) {
									...
									String count = String.valueOf(rst.getInt("published_count"));
									String birth = AID.dateToString(rst.getDate("date_of_birth"));
									
									String[] row = {id, first,last,nation,count,birth};
		...
		JLabel lblNewLabel_3_1_1 = new JLabel("Nationality");
		...
		nationf.setColumns(10);
		
		JLabel lblNewLabel_3_1_2 = new JLabel("Publish count");
		...
		addbtn.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try(Connection con = DriverManager.getConnection(url, myname, mypassword)) {
					...
					query = "INSERT INTO author ( first_name, last_name, nationality, published_count, date_of_birth) VALUES (?,?,?,?,?)";
					try (PreparedStatement pstmt = con.prepareStatement(query)){
		...
		updatebtn.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {				
				try(Connection con = DriverManager.getConnection(url, myname, mypassword)) {
					...
					query = "UPDATE author SET first_name = ?, last_name = ?, nationality = ?, published_count = ?, date_of_birth = ? WHERE id = ?";
					try (PreparedStatement pstmt = con.prepareStatement(query)){
			            pstmt.setString(1, first);
			            pstmt.setString(2, last);
			            pstmt.setString(3, nation);
			...
			public void actionPerformed(ActionEvent e) {
				try(Connection con = DriverManager.getConnection(url, myname, mypassword)) {
					...
					try (PreparedStatement pstmt = con.prepareStatement(query)){
						...
						i = pstmt.executeUpdate();
			           	if(i > 0) {	
			           		JOptionPane.showMessageDialog(null, "Author deleted Successfully", "Success", JOptionPane.INFORMATION_MESSAGE);
			           	}
			           	else
			           	{
			           		JOptionPane.showMessageDialog(null, "Error occur", "Error", JOptionPane.ERROR_MESSAGE);
					...
					.addGroup(gl_panel.createParallelGroup(Alignment.LEADING)
						.addGroup(gl_panel.createSequentialGroup()
							...
							.addGroup(gl_panel.createParallelGroup(Alignment.LEADING)
								...
								.addComponent(nationf, GroupLayout.DEFAULT_SIZE, 145, Short.MAX_VALUE)))
						.addGroup(gl_panel.createSequentialGroup()
							.addGap(29)
							.addGroup(gl_panel.createParallelGroup(Alignment.LEADING)
								.addComponent(lblNewLabel_3_1_2, GroupLayout.DEFAULT_SIZE, 145, Short.MAX_VALUE)
								.addComponent(countf, GroupLayout.DEFAULT_SIZE, 145, Short.MAX_VALUE)
		...
		gl_panel.setVerticalGroup(
			gl_panel.createParallelGroup(Alignment.LEADING)
				.addGroup(gl_panel.createSequentialGroup()
					...
					.addGap(6)
					.addComponent(nationf, GroupLayout.PREFERRED_SIZE, 31, GroupLayout.PREFERRED_SIZE)
					.addPreferredGap(ComponentPlacement.RELATED)
